...
## `next-auth/jwt`[](https://next-auth.js.org/getting-started/upgrade-v4#next-authjwt "Direct link to heading")
...
## `next-auth/react`[](https://next-auth.js.org/getting-started/upgrade-v4#next-authreact "Direct link to heading")
...
- `setOptions`: 더 이상 노출되지 않음, [`SessionProvider` props](https://next-auth.js.org/getting-started/client#options)를 사용하세요
- `options`: 더 이상 노출되지 않음, [`SessionProvider` props를 사용하세요](https://next-auth.js.org/getting-started/client#options)
...
## `SessionProvider`[](https://next-auth.js.org/getting-started/upgrade-v4#sessionprovider "Direct link to heading")
...
## 공급자[](https://next-auth.js.org/getting-started/upgrade-v4#providers "Direct link to heading")
...
사용법에 대한 자세한 내용은 OAuth 공급자 문서의 [옵션](https://next-auth.js.org/configuration/providers/oauth#options) 섹션을 참조하세요.
...
## `useSession` 훅[](https://next-auth.js.org/getting-started/upgrade-v4#usesession-hook "Direct link to heading")
...
`session.status`와 `session.data`에서 사용 가능한 값은 [문서를 확인하세요](https://next-auth.js.org/getting-started/client#usesession).
...
## 정의된 매개변수[](https://next-auth.js.org/getting-started/upgrade-v4#named-parameters "Direct link to heading")

콜백에 대한 인수를 명명된 매개변수 패턴으로 변경했습니다. 이렇게 하면 더미 `_` 자리 표시자나 다른 트릭을 사용할 필요가 없습니다.

### 콜백[](https://next-auth.js.org/getting-started/upgrade-v4#callbacks "Direct link to heading")
...
### 이벤트[](https://next-auth.js.org/getting-started/upgrade-v4#events "Direct link to heading")
...
## JWT 구성[](https://next-auth.js.org/getting-started/upgrade-v4#jwt-configuration "Direct link to heading")

우리는 JSON Web Tokens를 구성할때 일부 [구성옵션](https://next-auth.js.org/configuration/options)을 제거 하였으며, 자세한 내용은 [여기에서 확인해주세요](https://github.com/nextauthjs/next-auth/pull/3039).
...
## 로거 API[](https://next-auth.js.org/getting-started/upgrade-v4#logger-api "Direct link to heading")
...
## 테마[](https://next-auth.js.org/getting-started/upgrade-v4#theme "Direct link to heading")
...
새로운 테마 옵션에 대한 자세한 내용과 스크린샷은 [구성/페이지](https://next-auth.js.org/configuration/pages#theming)에서 확인할 수 있습니다.

[#2788](https://github.com/nextauthjs/next-auth/pull/2788)에서 소개되었습니다.

## 세션[](https://next-auth.js.org/getting-started/upgrade-v4#session "Direct link to heading")
...
3.  어댑터 사용, `strategy: "jwt"`: 사용자는 데이터베이스를 사용할 수 있는 경우에도 명시적으로 `next-auth`에 JWT를 사용하도록 지시할 수 있습니다. 이렇게 하면 보안이 저하되는 대신 조회 속도가 빨라질 수 있습니다. 자세히 알아보세요: [https://next-auth.js.org/faq#json-web-tokens](https://next-auth.js.org/faq#json-web-tokens)
...
## 어댑터[](https://next-auth.js.org/getting-started/upgrade-v4#adapters "Direct link to heading")
...
공식 어댑터는 기본 모노레포의 `packages` 디렉토리에서 찾을 수 있습니다([nextauthjs/next-auth](https://github.com/nextauthjs/next-auth)). 새로운 [간소화된 어댑터 API](https://github.com/nextauthjs/next-auth/pull/2361)를 사용하여 [직접 생성](https://next-auth.js.org/tutorials/creating-a-database-adapter)할 수도 있습니다.
...
### 어댑터 API[](https://next-auth.js.org/getting-started/upgrade-v4#adapter-api "Direct link to heading")
...
### 스키마 변경[](https://next-auth.js.org/getting-started/upgrade-v4#schema-changes "Direct link to heading")
...
## `secret` 누락[](https://next-auth.js.org/getting-started/upgrade-v4#missing-secret "Direct link to heading")

NextAuth.js는 사용자가 정의하지 않은 경우 편의성을 위해 비밀(secret)을 생성하였습니다. 이는 개발 환경에서는 유용할 수 있지만, 운영 환경에서는 문제가 될 수 있습니다. 우리는 문서에서 이에 대해 항상 명확히 밝혀왔지만, 이제부터는 운영 환경에서 `secret` 속성을 정의하지 않으면 사용자에게 오류 페이지를 표시합니다. 이 옵션에 대한 자세한 내용은 [여기](https://next-auth.js.org/configuration/options#secret)에서 확인할 수 있습니다.
...
## 세션 `strategy`[](https://next-auth.js.org/getting-started/upgrade-v4#session-strategy "Direct link to heading")

우리는 항상 두 가지 다른 세션 전략을 지원해왔습니다. 첫 번째는 가장 인기 있고 기본 전략인 JWT 기반 전략입니다. 두 번째는 데이터베이스 어댑터 지속 세션 전략입니다. 두 전략 모두 장단점이 있으며, 더 많은 내용을 알고 싶으시면 [FAQ](https://next-auth.js.org/faq) 페이지를 확인하시기 바랍니다.
...
자세한 내용은 [`session` 옵션 문서](https://next-auth.js.org/configuration/options#session)를 참조하십시오.
